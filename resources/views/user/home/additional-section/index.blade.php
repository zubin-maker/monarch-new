        ...
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
              <a href="{{ route('user.additional_section.create') }}" class="btn btn-primary btn-sm float-right"><i
                  class="fas fa-plus"></i> {{ __('Add') }}</a>

              <button class="btn btn-danger btn-sm float-right mr-2 d-none bulk-delete"
                data-href="{{ route('user.additional_section.bulkdelete') }}">
                    ...
                    <tbody>
                      @foreach ($sections as $sectoin)
                        <tr>
                          ...
                          <td>
                            {{ __('After') . ' ' . ucfirst(str_replace('_', ' ', @$sectoin->possition)) }}
                          </td>
                          <td>
                            <a class="btn btn-secondary btn-sm  mt-1 mr-1"
                              href="{{ route('user.additional_section.edit', ['id' => $sectoin->id]) }}">
                            ...
                            <form class="deleteForm d-inline-block"
                              action="{{ route('user.additional_section.delete', ['id' => $sectoin->id]) }}"
                              method="post">
                              @csrf
