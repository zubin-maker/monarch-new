...
@section('content')
  <div class="page-header">
    <h4 class="page-title">{{ __('Edit User Dashboard Keywords') }}</h4>
    <ul class="breadcrumbs">
      ...
      <li class="nav-item">
        <a href="#">{{ __('Edit User Dashboard Keywords') }}</a>
      </li>
    </ul>
  </div>
  <div class="row">
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <div class="card-title d-inline-block">{{ __('Edit User Dashboard Keywords') }}</div>
        ...
        <div class="card-body pt-5 pb-5" id="app">
          <div class="row">
            <div class="col-lg-12">
              @if (!is_null($json))
                <form method="post"
                  action="{{ !empty($la) ? route('admin.language.user_dashboard.updateKeyword', $la->id) : route('admin.language.user_dashboard.updateKeyword', 0) }}"
                  id="langForm">
                  {{ csrf_field() }}
              ...
              @else
                <h4 class="text-center text-danger">
                  {{ __('File does not exist. Please provide a valid language code.') }}</h4>
              @endif
            </div>
          </div>
        </div>
