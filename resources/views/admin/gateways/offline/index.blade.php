        ...
        <div class="card-header">
          <div class="row">
            ...
            <div class="col-lg-6 mt-2 mt-lg-0">
              <a href="#" class="btn btn-primary {{ $default->rtl == 1 ? 'float-lg-left' : 'float-lg-right' }} float-left btn-sm" data-toggle="modal"
                        ...
                        <tr>
                          ...
                          <td>
                            <form id="productForm{{ $ogateway->id }}" class="d-inline-block"
                              action="{{ route('admin.offline.status') }}" method="post">
                              @csrf
                              <input type="hidden" name="ogateway_id" value="{{ $ogateway->id }}">
                              <input type="hidden" name="type" value="product">
                              <select
                                class="w-min-max-100 form-control form-control-sm {{ $ogateway->status == 1 ? 'bg-success' : 'bg-danger' }}"
                                name="status"
                                onchange="document.getElementById('productForm{{ $ogateway->id }}').submit();">
                                <option value="1" {{ $ogateway->status == 1 ? 'selected' : '' }}>
                                  {{ __('Active') }}</option>
                                <option value="0" {{ $ogateway->status == 0 ? 'selected' : '' }}>
                                  {{ __('Deactive') }}</option>
                              </select>
                            </form>
                          </td>
                          ...
                          <td>
                            ...
                            <form class="deleteform d-inline-block" action="{{ route('admin.offline.gateway.delete') }}"
                              method="post">
                              @csrf
                              <input type="hidden" name="ogateway_id" value="{{ $ogateway->id }}">
